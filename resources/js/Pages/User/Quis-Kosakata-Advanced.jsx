...
export default function VocabularyQuizPage() {
  ...
  const handleAnswerSelect = async (optionId) => {
    ...
    try {
      await fetch(route('save-quis-kosakata-answer'), {
        ...
        body: JSON.stringify({
          ...
          selectedAnswer: optionId.toLowerCase(),
          isCorrect,
          waktuJawab,
    ...
    const newAnswer = {
      ...
      isCorrect,
      question: soal.soal_arti,
      kosakata: soal.kosakata_id,
  ...
  const handleViewReview = () => {
    // Redirect ke halaman review quis kosakata advanced
    router.visit(route('review-quis-kosakata-advanced', { sessionId }));
  }
  ...
  // Render opsi jawaban advanced
  const renderOptions = (soal, onSelect, selected, isAnswered, showFeedback) => {
    return (
      <div className="grid grid-cols-1 gap-6"> {/* 1 kolom, 1 baris per opsi */}
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="relative z-10 p-8 flex flex-col gap-2 items-center justify-center min-h-[90px]">
                  <span className="text-3xl md:text-4xl font-extrabold tracking-wide select-none">{option.kanji}</span>
                  {!hideFurigana && option.furigana && <span className="text-lg md:text-xl text-indigo-200 select-none">{option.furigana}</span>}
                  {!hideRomaji && option.romaji && <span className="text-base md:text-lg text-indigo-300 select-none">{option.romaji}</span>}
                  {showResult && (
                    <motion.div initial={{ scale: 0, rotate: -90 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 0.2, type: "spring", stiffness: 200 }} className="bg-white dark:bg-slate-800 rounded-full p-2 mt-2">
                ...
                <AccordionItem value="item-1">
                  <AccordionTrigger className="text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-lg px-4 py-3 hover:from-indigo-100 hover:to-purple-100 dark:hover:from-indigo-800/30 dark:hover:to-purple-800/30 transition-all duration-300">
                    <div className="flex items-center gap-3">
                      ...
                      <span>Panduan Kuis Kosakata Advanced</span>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="text-gray-600 dark:text-gray-400">
                    <div className="space-y-4 p-4 bg-gradient-to-br from-white/50 to-indigo-50/50 dark:from-slate-800/50 dark:to-indigo-900/50 rounded-lg border border-indigo-100 dark:border-indigo-800">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-800 dark:text-gray-200 mb-2">Selamat datang di Kuis Kosakata Level Advanced!</p>
                          <p className="text-sm">Pilih kalimat yang paling natural dan benar untuk penggunaan kosakata dalam konteks sehari-hari.</p>
                        </div>
                      </div>
                      <ul className="list-none space-y-3 ml-8">
                        ...
                        <li className="flex items-center gap-2">
                          ...
                          <span>Setiap soal berupa instruksi dalam bahasa Indonesia</span>
                        </li>
                        <li className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                          <span>Pilih kalimat yang paling natural dan benar</span>
                        </li>
                        <li className="flex items-center gap-2">
                    ...
                    <div className="text-center mb-8">
                      ...
                      >
                        <div className="mb-4 text-lg md:text-xl font-semibold text-indigo-700 dark:text-indigo-200 text-center">
                          Pilih kalimat yang paling natural untuk penggunaan kosakata berikut!
                        </div>
                        <div className="bg-gradient-to-br from-white via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-indigo-900 backdrop-blur-sm rounded-md shadow-md p-8 mb-6 border border-white/30 dark:border-slate-700/30 relative overflow-hidden">
                          ...
                          <div className="relative z-10">
                            ...
                            >
                              {quizData[currentQuestion].soal_arti}
                            </motion.h2>
                          </div>
                        </div>
                      </motion.div>
                          ...
                          >
                            {isAnswered && answers[answers.length - 1]?.isCorrect ? (
                              <>
                                ...
                                Benar! Jawaban: {quizData[currentQuestion].options.find(o => o.isCorrect)?.kanji}
                              </>
                            ) : (
                              <>
                                <XCircle className="w-6 h-6" />
                                Salah. Jawaban: {quizData[currentQuestion].options.find(o => o.isCorrect)?.kanji}
                              </>
                            )}
                          </div>
                        </motion.div>
