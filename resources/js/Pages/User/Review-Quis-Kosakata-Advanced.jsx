...
function ReviewQuisPage({ quizResults, user, currentLevel, currentExp, maxExp, nextLevelExp }) {
                    ...
                    <Accordion type="multiple" value={expandedItems} onValueChange={setExpandedItems}>
                      {quizResults.answers.map((answer, idx) => (
                        <AccordionItem key={idx} value={String(idx)}>
                          <AccordionTrigger
                            className="transition-colors duration-150 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 rounded-lg px-2 no-underline hover:no-underline"
                          >
                            <div className="flex items-center gap-4 w-full text-left">
                              {/* Label nomor bulat */}
                              <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold shadow-lg no-underline hover:no-underline ${
                                  answer.isCorrect
                                    ? "bg-gradient-to-br from-green-400 to-green-600 text-white"
                                    : "bg-gradient-to-br from-red-400 to-red-600 text-white"
                                }`}
                              >
                                <span className="no-underline hover:no-underline">{idx + 1}</span>
                              </div>
                              <div className="flex flex-col gap-1">
                                {/* Kosakata: kanji + furigana, fallback romaji */}
                                <span className="font-bold text-lg no-underline hover:no-underline">
                                  {answer.kosakata_label_kanji}
                                  {answer.kosakata_label_furigana ? (
                                    <span className="text-base text-gray-500 dark:text-gray-400 ml-2 no-underline hover:no-underline">{answer.kosakata_label_furigana}</span>
                                  ) : answer.kosakata_label_romaji ? (
                                    <span className="text-base text-gray-500 dark:text-gray-400 ml-2 no-underline hover:no-underline">{answer.kosakata_label_romaji}</span>
                                  ) : null}
                                </span>
                                {/* Soal (arti) */}
                                <span className="text-sm text-gray-600 dark:text-gray-300 no-underline hover:no-underline">{answer.soal_arti}</span>
                                {/* Badge EXP */}
                                <span className="mt-1 no-underline hover:no-underline">
                                  <span className="inline-flex items-center px-2 py-0.5 rounded bg-yellow-100 text-yellow-800 text-xs font-semibold dark:bg-yellow-900 dark:text-yellow-200 no-underline hover:no-underline">
                                    +{answer.expGained} EXP
                                  </span>
                                </span>
                              </div>
                            </div>
  ...
  <div className="space-y-6 p-4">
    ...
    <div>
      ...
      <div className="rounded-xl border bg-white dark:bg-slate-900 shadow-sm p-6 flex flex-col items-start mb-4">
        <span className="text-base font-semibold text-gray-500 mb-1">Instruksi</span>
        <span className="text-2xl md:text-3xl font-bold text-indigo-700 dark:text-yellow-200 tracking-wide leading-relaxed">{answer.soal_arti}</span>
      </div>
    </div>
    ...
    <div>
      ...
      {answer.options.find(opt => opt.id === answer.userAnswer) ? (
        <div className={`p-4 rounded-xl border-2 flex flex-col gap-1 ${
          ...
          <span className="font-bold mr-2 text-lg md:text-xl">{answer.options.find(opt => opt.id === answer.userAnswer).id.toUpperCase()}.</span>
          ...
          <span className="text-gray-700 dark:text-gray-300">{answer.options.find(opt => opt.id === answer.userAnswer).arti}</span>
        </div>
      ) : (
        <div className="p-4 rounded-xl border-2 border-gray-300 bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 dark:bg-slate-800 dark:text-gray-200">
    ...
    <div>
      ...
      {answer.correctAnswer ? (
        <div className="p-4 rounded-xl border-2 flex flex-col gap-1 border-green-300 bg-green-50 dark:border-green-700 dark:bg-green-900/40">
          <span className="font-bold mr-2 text-lg md:text-xl">{answer.correctAnswer.id.toUpperCase()}.</span>
          ...
          <span className="text-gray-700 dark:text-gray-300">{answer.correctAnswer.arti}</span>
        </div>
      ) : null}
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {answer.options.map((option, idx) => {
          ...
          if (option.id === answer.userAnswer && option.id === answer.correctAnswer?.id) className += " border-green-400 bg-green-50 dark:border-green-700 dark:bg-green-900/40 font-bold";
          else if (option.id === answer.userAnswer && option.id !== answer.correctAnswer?.id) className += " border-red-400 bg-red-50 dark:border-red-700 dark:bg-red-900/40 font-bold";
          else if (option.id === answer.correctAnswer?.id) className += " border-green-300 bg-green-100 dark:border-green-700 dark:bg-green-900/40";
          else className += " border-gray-200 bg-white dark:bg-slate-800";
          return (
            <div key={option.id} className={className}>
              <span className="font-bold mr-2 text-lg md:text-xl">{option.id.toUpperCase()}.</span>
              ...
              <span className="text-gray-700 dark:text-gray-300">{option.arti}</span>
            </div>
          );
        })}
      </div>
